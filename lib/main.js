...
function createList(items, count, includeWorld) {
   let moddedCount = count <= 0 ? -1 : count;
   if (includeWorld) ++moddedCount;
   return items
      .toArray()
      ...
      .map(el => cheerio(el).toArray().map(item => getClean(stripCommaOrNA(cheerio(item).text().trim()))))
...
function getSingleCountryInformation(page) {
   ...
   const mainNumbers = page('#maincounter-wrap').toArray().map(el => stripCommaOrNA(cheerio(el).text()).split(':')[1]).filter(el => el) || [];
